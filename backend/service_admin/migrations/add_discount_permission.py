"""
RBAC Permissions Migration - V3.0 Task A4: Discount Management
================================================================

Kedvezmények kezelésére vonatkozó jogosultságok hozzáadása.

Használat:
    python -m backend.service_admin.migrations.add_discount_permission

Ez a szkript idempotens - újrafuttatható, nem hoz létre duplikátumokat.
"""

import sys
import logging
from sqlalchemy.orm import Session
from sqlalchemy.exc import IntegrityError

# Import database models
from backend.service_admin.models.database import SessionLocal, init_db
from backend.service_admin.models.permission import Permission
from backend.service_admin.models.role import Role

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)
logger = logging.getLogger(__name__)


# ============================================================================
# DISCOUNT PERMISSIONS TO ADD
# ============================================================================

DISCOUNT_PERMISSIONS = [
    {
        "name": "discounts:apply",
        "display_name": "Kedvezmények alkalmazása",
        "description": "Kedvezmények alkalmazása rendelésekre és tételekre (manager jogosultság)",
        "resource": "discounts",
        "action": "apply",
        "is_system": True
    },
    {
        "name": "discounts:view",
        "display_name": "Kedvezmények megtekintése",
        "description": "Alkalmazott kedvezmények megtekintése",
        "resource": "discounts",
        "action": "view",
        "is_system": True
    },
    {
        "name": "discounts:manage",
        "display_name": "Kedvezmények kezelése",
        "description": "Kedvezmények teljes körű kezelése és adminisztrációja",
        "resource": "discounts",
        "action": "manage",
        "is_system": True
    }
]

# Permissions to add to Manager role
MANAGER_PERMISSIONS = [
    "discounts:apply",
    "discounts:view"
]

# Permissions to add to Admin role
ADMIN_PERMISSIONS = [
    "discounts:apply",
    "discounts:view",
    "discounts:manage"
]


# ============================================================================
# MIGRATION FUNCTIONS
# ============================================================================

def add_permissions(db: Session) -> dict:
    """
    Hozzáadja a kedvezmény jogosultságokat.

    Args:
        db: SQLAlchemy Session

    Returns:
        dict: Létrehozott/meglévő Permission objektumok name alapján
    """
    logger.info("🔐 Kedvezmény jogosultságok hozzáadása...")

    stats = {"added": 0, "skipped": 0}
    added_permissions = {}

    for perm_data in DISCOUNT_PERMISSIONS:
        # Ellenőrizzük, hogy már létezik-e
        existing = db.query(Permission).filter(
            Permission.name == perm_data["name"]
        ).first()

        if existing:
            logger.info(f"  ⏭️  Permission már létezik: {perm_data['name']}")
            stats["skipped"] += 1
            added_permissions[perm_data["name"]] = existing
            continue

        # Új permission létrehozása
        permission = Permission(
            name=perm_data["name"],
            display_name=perm_data["display_name"],
            description=perm_data["description"],
            resource=perm_data["resource"],
            action=perm_data["action"],
            is_system=perm_data["is_system"]
        )

        db.add(permission)
        added_permissions[perm_data["name"]] = permission
        stats["added"] += 1
        logger.info(f"  ✅ Létrehozva: {perm_data['name']}")

    db.commit()
    logger.info(f"✅ Permissions feldolgozva: {stats['added']} új, {stats['skipped']} meglévő")

    return added_permissions, stats


def update_role(db: Session, role_name: str, permission_names: list, permissions_map: dict) -> int:
    """
    Frissíti egy role jogosultságait.

    Args:
        db: SQLAlchemy Session
        role_name: Role neve (pl. "Manager", "Admin")
        permission_names: Hozzáadandó permission nevek listája
        permissions_map: Permission objektumok name alapján indexelve

    Returns:
        int: Hozzáadott jogosultságok száma
    """
    logger.info(f"👤 {role_name} role frissítése...")

    # Role lekérése
    role = db.query(Role).filter(Role.name == role_name).first()

    if not role:
        logger.warning(f"  ⚠️  {role_name} role nem található!")
        return 0

    added_count = 0

    # Új jogosultságok hozzáadása
    for perm_name in permission_names:
        if perm_name not in permissions_map:
            logger.warning(f"  ⚠️  Permission nem található: {perm_name}")
            continue

        permission = permissions_map[perm_name]

        # Ellenőrizzük, hogy már hozzá van-e rendelve
        if permission in role.permissions:
            logger.info(f"  ⏭️  {role_name} már rendelkezik: {perm_name}")
            continue

        role.permissions.append(permission)
        added_count += 1
        logger.info(f"  ✅ Hozzáadva {role_name}-hez: {perm_name}")

    db.commit()
    logger.info(f"✅ {role_name} role frissítve: {added_count} új jogosultság")

    return added_count


def verify_migration(db: Session):
    """
    Ellenőrzi a migráció eredményét.

    Args:
        db: SQLAlchemy Session
    """
    logger.info("\n📊 Migráció eredményeinek ellenőrzése...")

    # Permissions ellenőrzése
    logger.info("\n  Létrehozott permissions:")
    for perm_data in DISCOUNT_PERMISSIONS:
        perm = db.query(Permission).filter(
            Permission.name == perm_data["name"]
        ).first()

        if perm:
            logger.info(f"    ✅ {perm_data['name']}")
        else:
            logger.error(f"    ❌ {perm_data['name']} - HIÁNYZIK!")

    # Manager role ellenőrzése
    manager_role = db.query(Role).filter(Role.name == "Manager").first()
    if manager_role:
        manager_perm_names = [p.name for p in manager_role.permissions]
        logger.info(f"\n  Manager role jogosultságok:")
        for perm_name in MANAGER_PERMISSIONS:
            if perm_name in manager_perm_names:
                logger.info(f"    ✅ {perm_name}")
            else:
                logger.warning(f"    ⚠️  {perm_name} - HIÁNYZIK!")
    else:
        logger.warning("  ⚠️  Manager role nem található!")

    # Admin role ellenőrzése
    admin_role = db.query(Role).filter(Role.name == "Admin").first()
    if admin_role:
        admin_perm_names = [p.name for p in admin_role.permissions]
        logger.info(f"\n  Admin role jogosultságok:")
        for perm_name in ADMIN_PERMISSIONS:
            if perm_name in admin_perm_names:
                logger.info(f"    ✅ {perm_name}")
            else:
                logger.warning(f"    ⚠️  {perm_name} - HIÁNYZIK!")
    else:
        logger.warning("  ⚠️  Admin role nem található!")

    logger.info("\n✅ Migráció ellenőrzés befejezve")


# ============================================================================
# MAIN FUNCTION
# ============================================================================

def main():
    """
    Fő migráció folyamat.
    """
    logger.info("=" * 70)
    logger.info("🚀 Discount Permissions Migration - V3.0 Task A4")
    logger.info("=" * 70)

    try:
        # 1. Adatbázis inicializálás
        logger.info("\n1️⃣  Adatbázis kapcsolat...")
        init_db()
        logger.info("✅ Adatbázis kapcsolat OK")

        # 2. Session létrehozása
        db = SessionLocal()

        try:
            # 3. Permissions hozzáadása
            logger.info("\n2️⃣  Permissions hozzáadása...")
            permissions_map, stats = add_permissions(db)

            # 4. Manager role frissítése
            logger.info("\n3️⃣  Manager role frissítése...")
            manager_updated = update_role(db, "Manager", MANAGER_PERMISSIONS, permissions_map)

            # 5. Admin role frissítése
            logger.info("\n4️⃣  Admin role frissítése...")
            admin_updated = update_role(db, "Admin", ADMIN_PERMISSIONS, permissions_map)

            # 6. Ellenőrzés
            logger.info("\n5️⃣  Ellenőrzés...")
            verify_migration(db)

            logger.info("\n" + "=" * 70)
            logger.info("🎉 MIGRÁCIÓ SIKERES!")
            logger.info("=" * 70)
            logger.info(f"\n📊 ÖSSZESÍTÉS:")
            logger.info(f"  • Új permissions: {stats['added']}")
            logger.info(f"  • Meglévő permissions: {stats['skipped']}")
            logger.info(f"  • Manager role új jogosultságok: {manager_updated}")
            logger.info(f"  • Admin role új jogosultságok: {admin_updated}")
            logger.info("")

        finally:
            db.close()

    except Exception as e:
        logger.error(f"❌ HIBA a migráció során: {str(e)}")
        logger.exception(e)
        sys.exit(1)


if __name__ == "__main__":
    main()
