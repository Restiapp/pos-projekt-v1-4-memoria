...
# Phase D1: Helper Schemas
class AddItemsRequest(BaseModel):
    round_number: int = Field(default=1, ge=1)
    items: List[dict] # List of simplified item dicts {product_id, quantity, is_urgent...}
...
@orders_router.post(
    ...
    description="Returns existing OPEN order for table, or creates new one."
)
def open_order_for_table(
    table_id: int,
    db: Session = Depends(get_db)
) -> OrderResponse:
    order = OrderService.open_order_for_table(db, table_id)
    return OrderResponse.model_validate(order)
